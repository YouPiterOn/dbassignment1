...
def nonrepeatable_read_demo():
    ...
    try:
        ...
        # Transaction 1: Commited Read Before
        ...
        print(f"Commited read before update: Alice's balance = {balance_repeatable_read}")
        ...
        # Transaction 1: Commited Read After
        cursor1.execute("SELECT balance FROM accounts WHERE name = 'Alice'")
        balance_repeatable_read = cursor1.fetchone()[0]

        print(f"Commited read after update: Alice's balance = {balance_repeatable_read}")
